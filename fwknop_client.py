...
def main():
    parser = argparse.ArgumentParser(
        description='fwknop client - Send Single Packet Authorization',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''Examples:
  Request access to port 80 (TCP):
    python3 fwknop_client.py -A 80 -p 62201

  Request access to port 53 (UDP):
    python3 fwknop_client.py -A 53 -P udp -p 62201

  Request access to port 443 with verbose output:
    python3 fwknop_client.py -A 443 -p 62201 -v

  Specify source IP and keepalive interval:
    python3 fwknop_client.py -A 80 -s 192.168.1.100 -k 120 (for 2 minutes)

  Test mode (no actual firewall changes):
    python3 fwknop_client.py -A 80 --test

  Use custom config file:
    python3 fwknop_client.py -A 22 -p 62201 -c custom_config.json
''')
    ...
    client = FwknopClient(config_file=args.config, port=args.access, 
